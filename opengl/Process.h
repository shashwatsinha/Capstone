...
public:
	...
	virtual void VOnInit(const char * resourceString) { m_state = RUNNING; }  // called during the first update; responsible for setting the initial state (typically RUNNING)
...
//	virtual void VOnAbort(void) { }  // called if the process is aborted (see below)
	...
	WeakProcessPtr AttachProcess(StrongProcessPtr p);
};
